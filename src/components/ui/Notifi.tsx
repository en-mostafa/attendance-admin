                ...
                <div className={`modal-dialog modal-dialog-top w-450px`} onClick={e => e.stopPropagation()}>
                    ...
                    <p className="notificationPara">{notif.description}</p>
                        <div className="buttonContainer">
                            <button type="button" className="AllowBtn" onClick={() => handleClick(notif.id, true)}>{pending ? <Spinner /> : t('end')}</button>
                            <button type="button" className="NotnowBtn" onClick={() => handleClick(notif.id, false)}>{t('reminder_again')}</button>
                        </div>
                    </div>
                </div>
            </div>
