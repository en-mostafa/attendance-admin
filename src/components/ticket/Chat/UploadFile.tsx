...
const UploadFile: React.FC<UploadFileProps> = ({
  ...
  const formInputRef = useRef<HTMLInputElement | null>(null); // input واقعی فرم
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      //  setError(t('Chat.allowed_files'));
      //  return;
      //}

      if (file.size > 5 * 1024 * 1024) {
        setError(t('Chat.allowed_space'));
        return;
      }
          ...
          {selectedFiles.map((file, index) => (
            ...
            >
              {file.type.startsWith("image/") ? (
                ...
                <div className="w-100px h-30px bg-secondary d-flex align-items-center justify-content-center rounded">
                  {file.name}
                </div>
              )}
