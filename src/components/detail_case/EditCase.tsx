...
const EditCase: React.FC<Props> = ({ caseID }) => {
  ...
  useEffect(() => {
    (async () => {
        ...
        if (caseID && !isNaN(+caseID)) {
            ...
            const TotalPrices = res.productOption.reduce((sum:number, item:any) => {
                return sum + item.singlePrice * item.count;
            }, 0);
    
            const initPercents = [0, 0, 0];
            res.invoice.forEach((item: any) => {
                // step is 1-3, map into 0-based index
                initPercents[item.step - 1] = +((item.percent / 100) * TotalPrices).toFixed();
            });
  ...
  async function handleSubmit() {
    ...
    const invoices: InvoiceStep[] = invoicePercents.map((percent, idx) => ({
      step: (idx + 1) as 1 | 2 | 3,
      percent: +((percent / grandTotal) * 100).toFixed(1),
    }));
